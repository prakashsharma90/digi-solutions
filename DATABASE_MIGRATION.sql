...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_pricing_plans_price_non_negative') THEN
        ALTER TABLE pricing_plans 
        ADD CONSTRAINT chk_pricing_plans_price_non_negative 
        CHECK (is_custom = TRUE OR (price IS NOT NULL AND price >= 0));
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_pricing_plans_billing_cycle') THEN
        ALTER TABLE pricing_plans 
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_pricing_plans_currency') THEN
        ALTER TABLE pricing_plans 
...
FOR ALL 
USING (
    -- Check for direct admin role claim
    (auth.jwt() -> 'app_metadata' ->> 'role' = 'admin')
    OR
    -- Or verify specific user UID matches an admin record, avoiding broad EXISTS if possible
    -- However, standard practice often uses app_metadata. 
    -- If we must check public.users table via uid:
