...
ALTER TABLE services ADD COLUMN IF NOT EXISTS sort_order INTEGER GENERATED BY DEFAULT AS IDENTITY;

-- Re-apply check constraint for status
ALTER TABLE services DROP CONSTRAINT IF EXISTS services_status_check;
-- Add constraint as NOT VALID first to avoid locking/failure on existing rows
ALTER TABLE services ADD CONSTRAINT services_status_check CHECK (status IN ('Draft', 'Published')) NOT VALID;
-- Validate it (will fail if bad data exists, but migration won't start with invalid state)
ALTER TABLE services VALIDATE CONSTRAINT services_status_check;
